...
def _log_job_state(import_job_state):
    ...
    # logging.debug("    prev_tasklist_ids = " + str(import_job_state.prev_tasklist_ids))
    # logging.debug("    parents_ids = " + str(import_job_state.parents_ids))
    # logging.debug("    sibling_ids = " + str(import_job_state.sibling_ids))
    
    # logging.debug("    prev_tasklist_name = " + str(import_job_state.prev_tasklist_name))
    # logging.debug("    prev_depth = " + str(import_job_state.prev_depth))
    
    # logging.debug("    sibling_id = " + str(import_job_state.sibling_id))
    # logging.debug("    parent_id = " + str(import_job_state.parent_id))
    # logging.debug("    tasklist_id = " + str(import_job_state.tasklist_id))

    # logging.debug("    data_row_num = " + str(import_job_state.data_row_num))
    # logging.debug("    num_of_imported_tasks = " + str(import_job_state.num_of_imported_tasks))
    # logging.debug("    num_tasklists = " + str(import_job_state.num_tasklists))
    logservice.flush()
    ...
    def post(self):
        ...
        if self.user_email:
            ...
            if self.process_tasks_job is None:
                ...
                if self.process_tasks_job.status in constants.JobStatus.STOPPED_VALUES:
                    logging.warning(fn_name + "<End> Nothing to do (Job Status: " + self.process_tasks_job.status + ")")
                    logservice.flush()
                    return
                    
                if self.process_tasks_job.status == constants.JobStatus.STARTING:
                ...
                self._log_job_progress() # Log job state info
                ...
                try:
                    ...
                    self._report_error("Error connecting to Tasks service: " + shared.get_exception_msg(e))
                    logging.debug(fn_name + "<End> (Error connecting to Tasks service)")
                    logservice.flush()
                    return
                    
                
                if self.process_tasks_job.job_type == 'import':
                    try:
                        # ===================================
                        #           Import tasks
                        # ===================================
                        ...
                        logging.exception(fn_name + "Error importing")
                        logservice.flush()
                        self._report_error("Error importing: " + shared.get_exception_msg(e))
                        logging.debug(fn_name + "<End> (Error importing)")
                        logservice.flush()
                        return
                else:
                    self._report_error("System error: Expected import job, but found export job")
        else:
    ...
    def _import_tasks(self):
        ...
        try:
            ...
            if self.process_tasks_job.pickled_import_state:
                ...
                import_job_state = pickle.loads(self.process_tasks_job.pickled_import_state)
                
                _log_job_state(import_job_state)
                
                # Skip first 'n' lines that were processed in a previous job
                logging.debug(fn_name + "Skipping first " + str(import_job_state.data_row_num) + " processed tasks")
                dummy_data_row_num = 0
                while dummy_data_row_num < import_job_state.data_row_num:
                    csv_reader.next()
                ...
                import_job_state.prev_tasklist_ids = {} # "Tasklist name" : "Tasklist ID"
                if import_method == constants.ImportMethod.DELETE_BEFORE_IMPORT:
                    # TODO: Delete all existing tasklists
                    ...
                    for tasklist in tasklists:
                        ...
                        import_job_state.prev_tasklist_ids[tasklist_title] = tasklist_id
            ...
            # ---------------------------------------------------------------------------------------------------------
            self.process_tasks_job.message = 'Import starting ...'
            self.process_tasks_job.job_progress_timestamp = datetime.datetime.now()
            self.process_tasks_job.status = constants.JobStatus.IMPORTING
            self._log_job_progress() # Log job state info
            self.process_tasks_job.put()
            ...
            for task_row_data in csv_reader:
                import_job_state.data_row_num = import_job_state.data_row_num + 1
                ...
                if len(task_row_data) != 9 or task_row_data.get('restkey'):
                    ...
                    self._report_error("Task " + str(import_job_state.data_row_num) + " has " + str(len(task_row_data)) + " columns, expected 9")
                ...
                if status not in ('completed', 'needsAction'):
                    self._report_error("Invalid status value \"" + str(status) + "\" in task number " + str(import_job_state.data_row_num))
                ...
                if task_row_data.has_key('depth'):
                    try:
                        ...
                        err_msg = "Invalid depth value [" + str(task_row_data['depth']) + "] for task " + str(import_job_state.data_row_num) + ", Exception: " + str(e)
                    # 'depth' is not part of the Tasks resource, so delete it from the dictionary
                    del(task_row_data['depth'])
                else:
                    err_msg = "No 'depth' property for imported task number " + str(import_job_state.data_row_num)
                if err_msg:
                    ...
                    return 
                if depth < 0:
                    logging.debug(fn_name + "Depth of task " + str(import_job_state.data_row_num) + " [" + str(depth) + "] is less than zero. Setting depth = 0") 
                    depth = 0
                ...
                if not tasklist_name:
                    self._report_error('Missing value for "tasklist_name" column in task number ' + str(import_job_state.data_row_num))
                ...
                if import_method == constants.ImportMethod.SKIP_DUPLICATE_TASKLIST and import_job_state.prev_tasklist_ids.get(tasklist_name):
                    # Tasklist exists, so do not import tasks in this tasklist
                    if self.is_test_user:
                        logging.debug(fn_name + "Skipping task number " + str(import_job_state.data_row_num) + " because tasklist '" + 
                            tasklist_name + "' already exists")
                        logservice.flush()
                    continue
                    
                if tasklist_name != import_job_state.prev_tasklist_name:
                    # Processing a new/different tasklist in the imported data
                    import_job_state.num_tasklists = import_job_state.num_tasklists + 1
                    existing_tasklist_id = import_job_state.prev_tasklist_ids.get(tasklist_name)
                    if existing_tasklist_id:
                        if import_method == constants.ImportMethod.REPLACE_TASKLIST_CONTENT:
                            ...
                            import_job_state.tasklist_id = existing_tasklist_id
                    ...
                    if import_method in constants.ImportMethod.CREATE_NEW_TASKLIST_VALUES:
                        ...
                        import_job_state.tasklist_id = result['id']
                        import_job_state.prev_tasklist_ids['title'] = import_job_state.tasklist_id
                        if self.is_test_user:
                            logging.debug(fn_name + "Created new Tasklist [" + tasklist['title'] + "], ID = " + import_job_state.tasklist_id)
                            logservice.flush()
                            
                            
                    # Importing into a new list, or new tasks into an existing list, so start with no parents or siblings
                    import_job_state.parents_ids = [''] 
                    import_job_state.sibling_ids = [] # New list, so start with no siblings
                    import_job_state.prev_depth = 0
                    
                    import_job_state.prev_tasklist_name = tasklist_name
                ...
                # Valid depth values:
                #   depth = 0                   Root task
                #   depth < import_job_state.prev_depth          Task moved back up the task tree
                #   depth == import_job_state.prev_depth         Sibling task (same parent as previous task)
                #   depth == import_job_state.prev_depth + 1     New child task
                # Task depth must not be more than import_job_state.prev_depth + 1
                # List of import_job_state.parents_ids is updated after task has been added, because the current task may be the parent of the next task
                if depth > import_job_state.prev_depth+1:
                    # Child can only be immediate child of previous task (i.e., previous depth + 1)
                    self._report_error("Depth [" + str(depth) + "] of task " + str(import_job_state.data_row_num) + " is more than 1 below greater than previous task's depth [" + str(import_job_state.prev_depth) + "]")
                ...
                try:
                    parent_id = import_job_state.parents_ids[depth]
                except:
                    self._report_error("Invalid depth [" + str(depth) + "] for task " + str(import_job_state.data_row_num))
                    logging.info(fn_name + "Invalid depth [" + str(depth) + "] for task " + str(import_job_state.data_row_num) + "; Unable to determine parent. Previous task's depth was " + str(import_job_state.prev_depth) + " Exception: " + shared.get_exception_msg())
                    logging.debug(fn_name + "<End> due to invalid depth value")
                    logservice.flush()
                    return 
                ...
                if depth == import_job_state.prev_depth+1:
                    ...
                    import_job_state.sibling_id = ''
                elif depth+1 > len(import_job_state.sibling_ids):
                    # First task at this depth
                    import_job_state.sibling_id = ''
                else:
                    # Previous task at this depth
                    import_job_state.sibling_id = import_job_state.sibling_ids[depth]
                ...
                # ================================================================
                # logging.debug(fn_name + "Inserting task " + str(import_job_state.data_row_num) + " with depth " + str(depth) + " ==>")
                # logging.debug(task)
                # logservice.flush()
                ...
                while retry_count > 0:
                    ...
                    # ------------------------------------------------------
                    if import_job_state.num_of_imported_tasks == import_job_state.data_row_num:
                        msg = ''
                    else:
                        # Display previous num, because we haven't (yet) created this task
                        msg = "Imported " + str(import_job_state.num_of_imported_tasks) + " of " + str(self.process_tasks_job.total_num_rows_to_process) + " data rows."
                    ...
                    try:
                        ...
                        #       Create task
                        # =======================
                        result = self.tasks_svc.insert(tasklist=import_job_state.tasklist_id, body=task_row_data, parent=parent_id, previous=import_job_state.sibling_id).execute()
                        ...
                        # logservice.flush()
                        import_job_state.num_of_imported_tasks = import_job_state.num_of_imported_tasks + 1
                        # Succeeded, so continue
                        break
                    except apiclient_errors.HttpError, e:
                        logging.warning(fn_name + "HttpError inserting task number " +  str(import_job_state.data_row_num) + ". " + 
                            shared.get_exception_msg(e) + ", " + str(retry_count) + " retries remaining")
                        logservice.flush()
                        if e._get_reason().lower() == "daily limit exceeded":
                            self._report_error("Error creating task for data row " + str(import_job_state.data_row_num) + 
                                ": Daily limit exceeded. Please try again after midnight Pacific Standard Time.")
                            logging.debug(fn_name + "<End> (Daily Limit Exceeded)")
                            logservice.flush()
                            return
                    except Exception, e:
                        if retry_count > 0:
                            logging.warning(fn_name + "Error creating task for data row "+ str(import_job_state.data_row_num) + ": '" +
                                shared.get_exception_msg(e)  + "', " + str(retry_count) + " retries remaining")
                            logservice.flush()
                        else:
                            logging.exception(fn_name + "Still exception inserting task, after " + str(settings.NUM_API_TRIES) + " retries. Giving up.")
                            logging.debug(fn_name + "Failed task:" +
                                "\n    tasklist = " + str(import_job_state.tasklist_id) +
                                "\n    parent = " + str(parent_id) +
                                "\n    previous = " + str(import_job_state.sibling_id) +
                                "\n    Task num = " + str(import_job_state.data_row_num) +
                                "\n    Depth = " + str(depth) +
                                "\n    body = " + str(task_row_data))
                            logservice.flush()
                            
                            self._report_error("Error creating task number " + str(import_job_state.data_row_num) + ": " + shared.get_exception_msg(e))
                ...
                # --------------------------------------------
                # List of import_job_state.parents_ids is updated after task has been added, because the current task may be the parent of the next task
                if depth < import_job_state.prev_depth:
                    ...
                    del(import_job_state.parents_ids[depth+1:])
                # Store ID of current task in at this depth, as it could be the parent of a future task
                if len(import_job_state.parents_ids) == depth+2:
                    import_job_state.parents_ids[depth+1] = task_id
                else:
                    import_job_state.parents_ids.append(task_id)
                
                # Store ID of this task as sibling for next task at this depth
                if len(import_job_state.sibling_ids) < depth+1:
                    # First task at this depth
                    import_job_state.sibling_ids.append(task_id)
                else:
                    # There was a previous task at this depth
                    import_job_state.sibling_ids[depth] = task_id

                import_job_state.prev_depth = depth
                ...
                if run_time > settings.MAX_WORKER_RUN_TIME:
                    ...
                    logservice.flush()
                    _log_job_state(import_job_state)
                    self.process_tasks_job.pickled_import_state = pickle.dumps(import_job_state)
                    self.process_tasks_job.total_progress = import_job_state.num_of_imported_tasks
                    self.process_tasks_job.job_progress_timestamp = datetime.datetime.now()
                    if import_job_state.num_of_imported_tasks == import_job_state.data_row_num:
                        self.process_tasks_job.message = ''
                    else:
                        self.process_tasks_job.message = "Imported " + str(import_job_state.num_of_imported_tasks) + " from " + str(import_job_state.data_row_num) + " data rows."
                    self._log_job_progress() # Log job state info
                    self.process_tasks_job.put()
            ...
            self.process_tasks_job.status = constants.JobStatus.IMPORT_COMPLETED
            self.process_tasks_job.total_progress = import_job_state.num_of_imported_tasks
            if import_job_state.num_of_imported_tasks == import_job_state.data_row_num:
                self.process_tasks_job.message = "Imported " + str(import_job_state.num_of_imported_tasks) + " tasks into " + \
                    str(import_job_state.num_tasklists) + " tasklists, in " + proc_time_str
            else:
                self.process_tasks_job.message = "Imported " + str(import_job_state.num_of_imported_tasks) + " of " + str(import_job_state.data_row_num) + \
                    " tasks into " + str(import_job_state.num_tasklists) + " tasklists, in " + proc_time_str
            self.process_tasks_job.job_progress_timestamp = datetime.datetime.now()
            self._log_job_progress() # Log job state info
            self.process_tasks_job.put()
            ...
            logging.info(fn_name + "Imported " + str(import_job_state.num_of_imported_tasks) + " tasks for " + str(self.user_email) +
                ", from " + str(import_job_state.data_row_num) + 
                " data rows into " + str(import_job_state.num_tasklists) + " tasklists, in " + proc_time_str)
            ...
            while retry_count > 0:
                ...
                try:
                    ...
                    if retry_count > 0:
                        ...
                        logging.exception(fn_name + "Still error retrieving list of tasklists after " + str(settings.NUM_API_TRIES) + " retries. Giving up")
                        logservice.flush()
                        raise e
    ...
    def _delete_tasklist_by_id(self, tasklist_id, tasklist_name = None):
        ...
        while retry_count > 0:
            ...
            try:
                ...
                if retry_count > 0:
                    logging.debug(fn_name + "Error " + action_str + " tasklist '" + str(tasklist_name) + "', id = " + str(tasklist_id) +
                        ", " + str() + " retries remaining: " + shared.get_exception_msg(e))
                else:
                    logging.exception(fn_name + "Error " + action_str + " tasklist '" + str(tasklist_name) + "', id = " + str(tasklist_id) +
                        ", even after " + str(settings.NUM_API_TRIES) + " retries")
                    raise e
